...
def main(strategy_1,strategy_2,parameter_1,parameter_2):
    ...
    parser.add_argument('--save_path', type=str, default='result/%s'%(parameter_2))
    parser.add_argument('--dis_metric', type=str, default='ours', help='distance metric')

    args = parser.parse_args() 
    args.outer_loop, args.inner_loop = get_loops(args.ipc) 
    args.device = 'cuda' if torch.cuda.is_available() else 'cpu'
    #args.dsa_param_stage_0 = ParamDiffAug()
    ...
    #wandb.config.dsa_strategy_stage_0 = args.dsa_strategy_stage_0
    ...
    if args.create_sd:
        ...
        """if args.twostage: # we add DAs on the original Data
            for DA in range(10): # we add 100 different random DAs here
                seed = seed +100
                image_DA = DiffAugment(images_all_original, args.dsa_strategy_stage_0, seed=seed, param=args.dsa_param_stage_0)
                images_all = torch.cat((images_all,image_DA), dim=0) 
                labels_all = torch.cat((labels_all,labels_all_original), dim=0)"""
        ...
        differ = torch.zeros(size=(num_classes*args.ipc, im_size[0], im_size[1]), dtype=torch.double, requires_grad=False, device=args.device)
    ...
    for it in range(args.Iteration+1):
        ...
        if it in eval_it_pool and args.evaluate_sd:
            ...
            ''' visualize and save '''
            if it%100 == 0:
                save_name = os.path.join(args.save_path, 'vis_syn_%s_strategy_stage_1=%s_%s_%s_%dipc_Seed=%s_iter%d.png'%(args.method, args.dsa_strategy_stage_1, args.dataset, args.model, args.ipc,parameter_1, it))
                ...
                if it != 0:
                    print("iter_",it, differ)
                    torch.save({'data': copy.deepcopy(differ.detach().cpu())}, os.path.join(args.save_path, 'differ_%s_%s_%s_%dipc_%dit.pt'%(args.dsa_strategy_stage_1, args.dataset, args.model, args.ipc, it)))
        ...
        if args.create_sd:
            ...
            for ol in range(args.outer_loop):
                ...
                for c in range(num_classes):
                    ...
                    if args.twostage and Random_DA < Threshold_Random_DA:  # we add DAs on the original Data
                        ...
                        img_real = DiffAugment(img_real_load, args.dsa_strategy_stage_1, seed=seed, param=args.dsa_param_stage_1)
                        lab_real = torch.ones((img_real.shape[0],), device=args.device, dtype=torch.long) * c
                    else:
                        img_real = get_images(c, args.batch_real)
                        lab_real = torch.ones((img_real.shape[0],), device=args.device, dtype=torch.long) * c
                    ...
                    """if args.dsa:
                        seed = seed +1
                        img_real = DiffAugment(img_real, args.dsa_strategy_stage_1, seed=seed, param=args.dsa_param_stage_1)
                        img_syn = DiffAugment(img_syn, args.dsa_strategy_stage_1, seed=seed, param=args.dsa_param_stage_1)"""
                ...
                image_syn_before = copy.deepcopy(image_syn.detach())
                ...
                image_syn_after = copy.deepcopy(image_syn.detach())
                differ_step = torch.sum(torch.abs(image_syn_before - image_syn_after), dim = 1, keepdim = False)
                differ = differ + differ_step
            ...
            if it == args.Iteration: # only record the final results
                ...
                torch.save({'data': copy.deepcopy(image_syn.detach().cpu())}, os.path.join(args.save_path, 'image_syn_%s_strategy_stage_1=%s_%s_%s_%dipc_Seed=%s.pt'%(args.method, args.dsa_strategy_stage_1, args.dataset, args.model, args.ipc,parameter_1)))
                torch.save({'data': copy.deepcopy(label_syn.detach().cpu())}, os.path.join(args.save_path, 'label_syn_%s_strategy_stage_1=%s_%s_%s_%dipc_Seed=%s.pt'%(args.method, args.dsa_strategy_stage_1, args.dataset, args.model, args.ipc,parameter_1)))
                torch.save({'data': copy.deepcopy(differ.detach().cpu())}, os.path.join(args.save_path, 'differ_%s_%s_%s_%dipc.pt'%(args.dsa_strategy_stage_1, args.dataset, args.model, args.ipc)))
                torch.save({'data': data_save, 'accs_all_exps': accs_all_exps, }, os.path.join(args.save_path, 'res_syn_%s_strategy_stage_1=%s_%s_%s_%dipc_Seed=%s.pt'%(args.method, args.dsa_strategy_stage_1, args.dataset, args.model, args.ipc,parameter_1)))
...
Test_list = {
    'strategy_1': ['none'], #'none','color','crop','cutout','flip','scale','rotate','color_crop_cutout_flip_scale_rotate'
    ...
    'parameter_2': ['CIFAR10'], #Dataset 'CIFAR10','MNIST','SVHN' 
}
